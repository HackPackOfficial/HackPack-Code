...
PanelColorController::PanelColorController(int pin, int numPixels)
  ...
  usr = {0, Hr24Time, displayLeadingZero, bootColor, bootBrightness, msg_col[0], msg_col[1], msg_col[2]};
}
...
/*void PanelColorController::handleLeadingZeroes(TimeKeeper& clock){
  // show zeros if clock is in timer mode
  if(clck_state == RUN_TIMER && millis() > clock.getClock().ringTime - 1000) return;

  // show zeroes on boot, when clock is unset
  if(!clock.getClock().timeSet) return;

  // set color of all zeroes to "off" in clock mode, and when setting is chosen
  if(!usr.showLeadZeroes && display_state == TIME){
    int zeroesIndex = 0;

    // loop to find first non-zero and save the index
    for(int i = 0; i < 6; i++){
      if(panels.panelSet[i] == '0'){
        zeroesIndex++;
      } else {
        break;
      }
    ...
    // set color of all leading zeroes to black
    for(int i = 0; i < zeroesIndex; i++){
      for(int j = 0; j < 7; j++){
        panels.displayColors[i][j][0] = 0;
        panels.displayColors[i][j][1] = 0;
        panels.displayColors[i][j][2] = 0;
      }
    }
  }
}*/
